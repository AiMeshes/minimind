...
1. **Sequence Packing**: the data pipeline packs multiple documents into fixed `2048`-token blocks by max-length compact packing — each document (`[bos]+tokens+[eos]`) is placed whole; when the next one no longer fits, the block is sealed in place (tail padded) and a new block starts, so documents never span blocks (over-long docs are truncated, keeping bos/eos). Tail padding labels are set to `-100` and excluded from training, and under varlen attention the pad segment is isolated as its own segment — GPU utilization stays maximal;
2. **Varlen Attention (flash-attn varlen)** — the correct partner of packing. `flash_attn_varlen_func` isolates causal attention per document via `cu_seqlens` (documents never attend across boundaries), with RoPE positions reset to 0 inside each document; boundaries are auto-derived from bos **and pad** positions in the packed block, no data-format change needed. Wrapped as a `torch.library` custom op, fully compatible with `torch.compile`;
